...
export default class GridT extends Component {
  constructor(props) {
    super(props);
    this.props = props;
    this.state = {
      imageList:
        [
          ...
          {
            imgUrl: require('./image/badBg.png'),
            textDesc: '背景复杂凌乱',
            bigOr: 0,
          },
          ...
          {
            imgUrl: require('./image/right.png'),
            textDesc: '正确姿势',
            bigOr: 1,
          },
        ...
        if (badge[i].bigOr == 0) {
          arr.push(
            <View style={styles.viewStyle}>
              <Image
                source={badge[i].imgUrl}
                style={styles.faceWidth}
                resizeMode="contain"
              />
          ...
          arr.push(
            <View style={styles.outerViewStyle}>
              <View style={styles.viewStyle}>
                <Image
                  source={badge[badge.length - 1].imgUrl}
                  style={styles.bigImage}
                  resizeMode="contain"
                />
                <Text style={styles.textStyle}>
                  {badge[badge.length - 1].textDesc}
                </Text>
              </View>
            </View>,
          );
  ...
  render() {
    return <View style={styles.container}>{this.generateDetailItems()}</View>;
  }
}
